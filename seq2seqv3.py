...
class Seq2SeqModel():
    """ Seq2Seq model using usign blocks from new 'tf.contrib.seq2seq'
    """
    PAD = 0
    EOS = 1
    ...
    def _init_placeholders(self):
        ...
        # required for trainning, not requird for testing
    ...
    def _init_decoder_train_connectors(self):
        ...
        with tf.name_scope('DecoderTrainFeeds'):
            ...
            # hacky way using one_hot to push EOS symbol at the end of target sequence
            decoder_train_targets = tf.add(decoder_train_targets,
                                           decoder_train_targets_eos_mask)

            self.decoder_train_targets = tf.add(decoder_train_targets,
                                                decoder_train_targets_eos_mask)

            self.loss_weights = tf.ones([batch_size,
    ...
    def _init_decoder(self):
        with tf.variable_scope("Decoder") as scope:
            # using one fully connected layer to generate outputs
            def output_fn(outputs):
                return tf.contrib.layers.linear(outputs, self.vocab_size, scope=scope)

            if not self.attention:
                 # sequence to sequence learning without attention mechanism
            ...
            (self.decoder_outputs_train,
             ...
             self.decoder_context_state_train) = (
                seq2seq.dynamic_rnn_decoder(
                    ...
                    inputs=self.decoder_fn_inputs_embedded,
